...
export class ChatService {
  ...
  private periodicCheck$ = interval(4000);
  ...
  constructor() {
    ...
    this.socket.on('new_message', (messages) => {
      console.log(messages);
      if (this.selectedChatSubject.value  === true) {
        console.log(messages[messages.length - 1]);

        this.seeMessage(messages[messages.length - 1].chat, JSON.parse(sessionStorage.getItem('user') || '{}')._id);
